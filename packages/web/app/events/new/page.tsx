  ...
  return (
    <section className="max-w-6xl space-y-6">
      ...
      <div className="space-y-3">
        <label className="eyebrow block">Quick filters</label>
        <div className="flex flex-wrap gap-1.5">
          {QUICK_FILTERS.map((f) => (
            <button
              key={f.label}
              onClick={() => search(f.query)}
              className={`rounded-full px-3 py-1 text-xs ${query === f.query ? 'bg-accent text-white' : 'bg-surface-2 text-ink-2 hover:bg-line'}`}
              type="button"
              title={f.query}
            >
              {f.label}
        ...
        <label className="eyebrow block pt-2">Search</label>
        <div className="flex gap-2">
          ...
          />
          <button onClick={() => search()} disabled={isPending} className="btn-primary">
      ...
      {err && (
        <p className="rounded-card bg-red-50 p-4 text-sm text-red-700 ring-1 ring-inset ring-red-600/20">{err}</p>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold">
              Results <span className="text-xs font-normal text-ink-3">({sortedResults.length})</span>
            </h3>
            <select
              value={sort}
          ...
          {sortedResults.length === 0 ? (
            <p className="card-quiet p-5 text-center text-xs text-ink-3">
              ...
              {sortedResults.map((m) => {
                ...
                return (
                  <li
                    key={m.id}
                    onClick={() => setPicked(m)}
                    className={`cursor-pointer rounded-sm p-2.5 text-sm ${isPicked ? 'bg-accent-soft ring-1 ring-inset ring-accent-line' : 'bg-surface ring-1 ring-inset ring-line hover:bg-surface-2'}`}
                  >
                    <div className="flex items-baseline justify-between gap-2">
                      <p className="truncate font-medium">{m.subject || '(no subject)'}</p>
                      <span className="flex-none text-xs text-ink-3">{formatGmailDate(m.internal_date)}</span>
                    </div>
                    <p className="truncate text-xs text-ink-2">{m.from}</p>
                    <p className="line-clamp-2 text-xs text-ink-3">{m.snippet}</p>
                  </li>
                );
              })}
            </ul>
        ...
        <div className="space-y-3">
          ...
          {!picked ? (
            <div className="card-quiet p-6 text-center text-xs text-ink-3">
            ...
            <article className="card overflow-hidden p-0">
              <header className="space-y-2 border-b border-line p-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 text-xs">
                    <p className="text-sm font-medium text-ink">{picked.from}</p>
                    <p className="text-ink-3">to me · {formatGmailDate(picked.internal_date)}</p>
                  </div>
                </div>
              </header>
              <div className="max-h-[500px] overflow-y-auto p-4 text-sm leading-relaxed">
                {previewLoading ? (
                  ...
                  <pre className="whitespace-pre-wrap break-words font-sans text-ink-3">{picked.snippet}</pre>
                )}
              </div>
            </article>
          )}
      ...
      {picked && (
        <div className="card space-y-4 p-5">
          ...
          <label className="block text-xs">
            ...
            <input className="field mt-1.5 w-full" placeholder="Event name" value={name} onChange={(e) => setName(e.target.value)} />
          </label>
          <label className="block text-xs">
            <span className="font-medium">Date &amp; time</span>
            <input type="datetime-local" className="field mt-1.5 w-full" value={date} onChange={(e) => setDate(e.target.value)} />
          </label>
          <label className="block text-xs">
            <span className="font-medium">Venue (optional)</span>
            <input className="field mt-1.5 w-full" placeholder="Venue" value={venue} onChange={(e) => setVenue(e.target.value)} />
          </label>
          <button onClick={submit} disabled={isPending || !name || !date} className="btn-primary bg-emerald-600 hover:bg-emerald-700">
